...
class SeriesMetadata:
    ...
    def post_process_after_loading(self):
        ...
        assert self.prefetch_step >= 0 and self.prefetch_step <= self.num_layers - 2, (
        ...
        for layer_idx in range(self.start_layer, self.end_layer):
            ...
            # If the weight uses dummy weight, make a copy temporary such that the
            # post method call won't affect other layers which also uses dummy weight.
            if not is_source:
            ...
            if step < self.prefetch_step:
                # Build the windows for the first `prefetch_step` layers.
                # The weights can be used for the first `prefetch_step` layers
                # in `forward()`, so also clone the weights.
                ...
                # When the layer not intended to be stored in this device,
                # link to the corresponding window's tensor.
                if not is_source:
                    layer.weight.set_(self.shared_windows[-1].weight)
            else:
                # Build one more window for prefetch. The weight is useless,
                # so just keep the shape.
                if step == self.prefetch_step:
                    self.shared_windows.append(
                        SharedWindowMetadata(
...
In a parallel group, each device stores a distinct, 

non-overlapping subset of layers from the series. 

All layers in a series must have the same structure (are isomorphic). 

The weight matrix for the i-th layer is stored on device (i % n), 

where n is the number of devices.

After loading the model, you must call 

`post_process_after_loading_for_shared_weight_series(layer)` 

on any layer of this series to complete the initialization.

During execution, each time a new layer is reached, 

you must call `reach_layer_for_shared_weight_series(layer)` 

for that layer to prefetch the weights. The argument `prefetch_step` 

is a non-negative integer k that manages asynchronous weight prefetching. 

Each call to `reach_layer_for_shared_weight_series(current_layer)` method 

will trigger an asynchronous prefetch for the weights of the k-th 

subsequent layer after `current_layer` within the series.

Note: The layers are managed as a circular buffer. 
The index of the layer to prefetch is determined by the formula:
- start_layer is the index of the first layer in the series (inclusive).
- end_layer is the index of the last layer in the series (exclusive). 
Thus, the series includes all layers with indices in the range 
[start_layer, end_layer).
- total_layers = end_layer - start_layer
- prefetch_layer_idx = (layer_idx + prefetch_step) % total_layers + start_layer

To hold the weights for the current layer and the k prefetched layers, 
a pool of (k + 1) shared tensor buffers will be created for this series.

Arguments:
    series_name: This name identifies which series this layer belongs to.
    group: The group coordinator for handling asynchronous communications. 
           It is recommended to create a new group coordinator for each new series.
    layer: The linear layer object to register.
    prefetch_step: An integer that manages asynchronous weight prefetching. 
                   Setting it to 0 or 1 can cover most cases.
"""
...
def register_layer_to_shared_weight_series(
    ...
    # Discard the original `process_weights_after_loading` method such that
    # it won't be called by others.
    layer.quant_method.process_weights_after_loading = lambda layer: None
    # When the layer not intended to be stored in this device, dispose the
    # tensor and skip weight loading.
